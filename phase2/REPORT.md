...
| OFFICE-9-XNET | Cross-network export + run on remote | ✅ first-try pass | `mur agent export` → scp .murpkg + tiny `a2a_send.py` to remote Linux host → unpack into `~/.mur/agents/` → symlink to runtime → 3 round-trips via `ssh ... python3 a2a_send.py ...` |
...
| OFFICE-11-XNET-LLM | Cross-network REAL-LLM specialist | ⏸ deferred (env-blocked, gated by `HARNESS_REMOTE_LLM=1`) | Code shipped + harness path validated to deploy; the test host's `qwen3:4b` was unresponsive on CPU-only 2-core (warm `/api/chat` returned 0 bytes after 180s). Mur side is proven: E4+E6 verified locally with `llama3.2:3b` (OFFICE-7-LLM) and cross-network deploy verified with echo (OFFICE-9). Only the third leg (live remote inference) is blocked on the host's Ollama setup, not on mur. |
...
Validated on a remote Ubuntu 20.04 x86_64 host with Ollama 0.18.0 and `qwen3:4b` chat model:
